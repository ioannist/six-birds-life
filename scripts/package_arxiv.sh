...
OUTPUT_PATH_INPUT="${2:-$REPO_DIR/arxiv_upload.tar.gz}"

if [[ "$MAIN_TEX_INPUT" = /* ]]; then
  MAIN_TEX="$MAIN_TEX_INPUT"
else
  MAIN_TEX="$REPO_DIR/$MAIN_TEX_INPUT"
fi
...
for f in sorted(files):
    try:
        ...
        print(f"WARNING: skipping file outside repo: {f}", file=sys.stderr)
        continue
    print(str(f))
PY
)
...
done < <(find "$REPO_DIR" -maxdepth 2 -type f \( -name "*.sty" -o -name "*.cls" -o -name "*.bst" -o -name "*.bbx" -o -name "*.cbx" \))

# De-duplicate
readarray -t UNIQUE_FILES < <(printf "%s\n" "${TEX_FILES[@]}" | awk 'NF' | sort -u)

# Copy files into temp dir preserving paths relative to repo.
